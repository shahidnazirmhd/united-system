...
│   │   └── endpoints/      # one file per backend module called: employees.py (profile reads AND Telegram linking, both apps.employees), leave.py (types/balance/apply/history/detail/cancel, mirrors LEAVE_API.md's telegram/ surface), approvals.py (pending/decide, mirrors APPROVALS_API.md's telegram/ surface — §3d)
│   ├── auth/                # conversation state this service keeps locally: account linking (no token storage — see §4), the multi-step Apply Leave flow (§3b), AND the one-step approval-decision comment flow (§3d)
...
## 3d. The Approval Engine (Phase 9)

This is the first module built on `apps.approvals` (backend) rather than a subject-specific module like Leave, and the first place this service receives an *unsolicited* push from the backend instead of only answering requests it initiated. The split, matching this document's usual endpoint/formatting/handler/auth layering:

- **`api_client/endpoints/approvals.py`** — `ApprovalsEndpoint`, mirroring `leave.py`'s shape exactly: `list_pending(*, telegram_user_id)` (GET `/api/v1/approvals/telegram/pending/`) and `decide(*, telegram_user_id, approval_request_id, decision, comments)` (POST `/api/v1/approvals/telegram/decide/`), both authenticated the same static-secret way every other Gateway→backend call is (§7). `ApprovalStep`/`ApprovalRequest` are frozen dataclasses with string ids, same convention as `leave.py`'s response types. The engine is subject-agnostic — `ApprovalRequest.subject_summary` is an opaque, backend-supplied display string (the same idiom `calendar_widget.py`'s `prompt`/`label` uses); this Gateway never interprets `subject_type` or `subject_id`, so a future Attendance/Overtime/Asset-Request approval needs zero changes here.
- **`formatting/approval_formatter.py`** — pure functions turning an `ApprovalRequest`/decision result into message text: `format_no_pending_approvals()`, `format_pending_approval_item(request)`, `format_approval_requested_push(*, subject_summary, level)`, `format_approval_decided_push(*, subject_summary, final_status, comments)`, `format_approval_step_advanced_push(*, message, subject_summary)` (Leave review round — see below), `format_approval_comment_prompt(decision)`, `format_approval_decision_result(request)` (handles both "still pending, advanced to the next level" and "final approved/rejected" outcomes, since a single decide() call can produce either — see the backend's own dynamic-level design).
- **`handlers/approval_handlers.py`** — `/pending_approvals` (`@registry.command(..., menu_label="✅ Pending Approvals", menu_order=40)`) sends one message per pending item, each with its own inline Approve/Reject row from `formatting/keyboards.build_approval_decision_keyboard(approval_request_id)`. Tapping either button is handled by `@registry.callback_prefix("approval:decide:approve:")`/`"approval:decide:reject:"` — same prefix-dispatch mechanism §6 describes for Leave's dynamic callback data — which answers the callback, clears that message's keyboard (so a manager can't double-tap the same decision), and starts a one-step "reply with an optional comment" mini-flow via `auth/approval_decision.py`'s `ApprovalDecisionService`.
- **`auth/approval_decision.py`** — `ApprovalDecisionService`, Redis-backed conversation state exactly like `auth/leave_application.py`'s multi-step flow but with a single step: `start(...)` records `{approval_request_id, decision}`; the manager's next free-text message is routed to `handle_approval_comment_free_text` (via `webhook/update_router.py`'s `_route_message`, gated on `ctx.approval_decision.is_active(...)`, checked right alongside the existing Leave-application-in-progress check); `submit_comment(...)` treats the literal text `"skip"` as no comment, calls `ApprovalsEndpoint.decide(...)`, and **always** clears the Redis state in a `finally` block — even if the backend call raises (e.g. the request was already decided by then), so a manager is never stuck unable to start a new decision.
- **The reverse-direction push — `POST /internal/notify`** (`webhook/server.py`) is the first time the backend calls *into* this service rather than the other way around. `apps.approvals`'s Celery task (`send_approval_notification`) POSTs here whenever a new approval request is created, a non-final decision advances the chain, or a decision is finalized, authenticated with the same `X-Internal-Service-Key` shared secret used in the opposite direction (§7) — this endpoint verifies it with the identical `hmac.compare_digest` discipline the backend's own `HasInternalServiceKey` permission uses, and rejects with 403 on any mismatch or missing header. Three `notification_type` values: `"approval_requested"` sends the (newly assigned) approver a push with the Approve/Reject keyboard attached (`format_approval_requested_push` + `build_approval_decision_keyboard`); `"approval_step_advanced"` (Leave review round) sends the *original requester* — not the new approver — a plain push with no keyboard, wrapping the backend's already-composed `message` field (`format_approval_step_advanced_push`; for Leave, level 1→2, this is exactly "Your manager has approved your leave request. It is now awaiting HR processing.") — never sent for a final decision; `"approval_decided"` sends the original requester a plain final-result message (`format_approval_decided_push`) once the whole chain concludes. Any other `notification_type` is a 422.
- **Nothing here is Leave-specific.** No file under `api_client/`, `formatting/`, `handlers/`, or `auth/` for this feature imports anything from `leave.py`/`leave_formatter.py`/`leave_application.py` — a future Attendance or Asset-Request approval reuses every one of these files unchanged, exactly as `apps.approvals` guarantees on the backend side.
- **Approval Workflow Changes review round — no code changes were needed anywhere in this service.** The backend added a per-step channel restriction (`ApprovalStep.restricted_to_channel`, `"web"` / `"telegram"` / `null` — see `APPROVALS_API.md`'s "Restricting a step to one channel" section) so that Leave's level 1 (manager) is Telegram-only and level 2 (HR/Admin) is web-only. `GET /api/v1/approvals/telegram/pending/` already filters server-side to steps decidable via Telegram, so `/pending_approvals` simply never lists a web-only step to begin with — nothing for this Gateway to additionally check. `POST /api/v1/approvals/telegram/decide/` already rejects a web-only step with `403 approval_channel_not_allowed` — `ApprovalsEndpoint.decide(...)` surfaces that as an ordinary backend error, handled by the same generic error-message path every other endpoint call already uses (no new error case to special-case here). In short: this Gateway never knew — and still doesn't know — that "Telegram" or "the web app" mean anything special; the backend enforces the restriction entirely on its own side, exactly as the "generic, subject-agnostic engine" design in `APPROVALS_API.md` intends.
...
| `/pending_approvals` | Yes | Approval requests awaiting this employee's decision as a manager, each with inline Approve/Reject buttons — see §3d |
...
| Reverse direction: the backend's Approval Engine push to `POST /internal/notify` authenticated the same way, same secret | `webhook/server.py`'s `internal_notify` route checks inbound `X-Internal-Service-Key` via `hmac.compare_digest`, 403s on mismatch/missing — see §3d |
...
**Leave (§3b) and the generic Approval Engine (§3d) are built; Attendance/Payroll are not.** Adding each of those never requires touching `webhook/update_router.py`'s dispatch logic or any existing handler file — see §6. The new files needed each time are `api_client/endpoints/<module>.py` and `handlers/<module>_handlers.py`, matching the backend's own module-boundary discipline (a handler for a Leave command only ever imports `endpoints/leave.py`, never another module's endpoint file). `handlers/context.py` and `webhook/server.py` do need additive edits each time — a new required field on `HandlerContext`, a new dependency wired in the composition root — as both Leave's and Approvals' own integration demonstrate. Any future date-entry step (an Attendance correction date, a Payroll effective date, ...) doesn't need any of that groundwork repeated — `handlers/calendar_widget.py` (§3c) is already generic; a new module just picks its own `purpose` string and registers a handler.

**The Approval Engine (§3d) is deliberately subject-agnostic**, so a future Attendance, Overtime, Business Trip, or Asset Request approval flow reuses `apps.approvals` on the backend and every file under `api_client/endpoints/approvals.py`, `formatting/approval_formatter.py`, `handlers/approval_handlers.py`, and `auth/approval_decision.py` on this side unchanged — the only new work for a future module is its own backend-side `ApprovalChainResolverPort` registration (see `HRMS_Architecture.md`/the backend's `apps/leave/infrastructure/leave_approval_chain_resolver.py` for the worked example) and, if it wants a manager to see a richer summary than the default, a well-formed `subject_summary` string when it creates the approval request. Attendance/Payroll as standalone employee-facing Telegram modules (their own commands like `/attendance_history`) remain unbuilt and out of scope until specified.
